...
var Goodreads = (function() {
  ...
  Goodreads = class Goodreads {
    ...
    showUser(username) {
      this.options.path = `${this.endpoint}/user/show.xml?key=${this.options.key}&username=${username}`;
    ...
    getAuthor(authorId, page) {
      ...
      this.options.path = `${this.endpoint}/author/list/${authorId}?key=${this.options.key}&page=${page}`;
      return this.getRequest();
    }
    ...
    getSeries(seriesId) {
      // Provide path to the API
      this.options.path = `${this.endpoint}/series/${seriesId}?key=${this.options.key}`;
      return this.getRequest();
    }
    ...
    getShelves(userId, callback) {
      // Provide path to the API
      this.options.path = `${this.endpoint}/shelf/list.xml?user_id=${userId}&key=${this.options.key}`;
      return this.getRequest();
    }
    ...
    searchBooks(q) {
      this.options.path = `${this.endpoint}/search/index.xml?key=${this.options.key}&q=${encodeURI(q)}`;
      return this.getRequest();
    }
    ...
    showAuthUser(accessToken, accessTokenSecret) {
      return new Promise((resolve, reject) => {
        let oa = new oauth(this.options.oauth_request_url, this.options.oauth_access_url, this.options.key, this.options.secret, this.options.oauth_version, this.options.callback, this.options.oauth_encryption)
        let parser = new xml2js.Parser()
        oa.get(`${this.endpoint}/api/auth_user`, accessToken, accessTokenSecret, (error, data, response) => {
          if (error) {
