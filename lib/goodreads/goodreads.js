(function() {
  ...
  Goodreads = (function() {
    ...
    Goodreads.prototype.consumer = function(options) {
      return new oauth.OAuth('http://goodreads.com/oauth/request_token', 'http://goodreads.com/oauth/access_token', options.key, options.secret, '1.0', options.callback, 'HMAC-SHA1');
    };
    ...
    Goodreads.prototype.requestToken = function(callback, req, res) {
      ...
      return this.consumer(this.options).getOAuthRequestToken(function(error, oauthToken, oauthTokenSecret, results) {
        if (error) {
          console.log(error);
          return callback('Error getting OAuth request token : ' + JSON.stringify(error), 500);
        } else {
          console.log;
          res.redirect('https://goodreads.com/oauth/authorize?oauth_token=' + oauthToken + '&oauth_callback=' + this.consumer()._authorize_callback);
          return callback(oauthToken);
        }
      });
    };
