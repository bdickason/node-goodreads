(function() {
  ...
  Goodreads = (function() {
    ...
    Goodreads.prototype.getFriends = function(userId, req, res, callback) {
      ...
      return oa.getProtectedResource(this.options.path, 'GET', req.session.goodreads_accessToken, req.session.goodreads_secret, function(error, data, response) {
        if (error) {
          console.log(oa);
    ...
    Goodreads.prototype.processCallback = function(oauthToken, oauthTokenSecret, callback) {
      var oa, parser;
      parser = new xml2js.Parser();
      console.log(oauthToken + oauthTokenSecret);
      oa = new oauth(this.options.oauth_request_url, this.options.oauth_access_url, this.options.key, this.options.secret, this.options.oauth_version, this.options.callback, this.options.oauth_encryption);
      oa.getOAuthAccessToken(oauthToken, oauthTokenSecret, function(error, oauthAccessToken, oauthAccessTokenSecret, results) {
        if (error) {
          return callback('Error getting OAuth access token : ' + (sys.inspect(error)) + '[' + oauthAccessToken + '] [' + oauthAccessTokenSecret + '] [' + (sys.inspect(results)) + ']', 500);
        } else {
          return oa.get('http://www.goodreads.com/api/auth_user', oauthToken, oauthTokenSecret, function(error, data, response) {
            if (error) {
